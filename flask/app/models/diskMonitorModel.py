...
class DiskMonitor(db.Model):
    ...
    total_space = db.Column(db.Float, nullable=False)  # 总空间（GB）
    used_space = db.Column(db.Float, nullable=False)  # 已用空间（GB）
    free_space = db.Column(db.Float, nullable=False)  # 可用空间（GB）
